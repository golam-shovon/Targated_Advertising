...
with open('data.csv', 'r') as movies_file:
...
for m in sparse_train_ratings_4plus[2].indices:
...
u2_features = sparse.csr_matrix(user_indicator_features)[2]
u2_rankings = hybrid_model.predict_rank(user_features=u2_features,
...
u2_top_ten_recs = numpy.where(u2_rankings <= 10)[0]
print("User 32 recommendations:")
for m in u2_top_ten_recs:
    print(movie_titles_by_internal_id[m])
# Print out User 432's held-out interactions
print("User 32's held-out movies:")
for m in sparse_test_ratings_4plus[2].indices:
    print(movie_titles_by_internal_id[m])
